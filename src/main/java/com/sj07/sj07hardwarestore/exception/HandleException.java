package com.sj07.sj07hardwarestore.exception;
...
public class HandleException extends ResponseEntityExceptionHandler implements ErrorController {
    ...
    public ResponseEntity<HttpResponse> handleMethodQueryException(QueryParameterException exception) {
        log.error(exception.getMessage());
        return new ResponseEntity<>(
                HttpResponse.builder()
                        .timeStamp(now().toString())
                        .reason(exception.getMessage().contains("Some Error") ? " " : exception.getMessage())
                        .build(), BAD_REQUEST);
    }
